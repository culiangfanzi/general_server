...
// AddTimeout adds a single request to commit by the BulkProcessorService.
//
// The caller is responsible for setting the index and type on the request.
func (p *BulkProcessor) AddTimeout(request BulkableRequest, t time.Duration) bool {
	select {
	case p.requestsC <- request:
		return true
	case <-time.After(t):
		return false
	}
